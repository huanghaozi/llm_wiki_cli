 ...
 *
 * Three real-corpus shapes the model reproducibly emits:
 *
 *   1. The whole page wrapped in a fenced code block:
 *        ```yaml
 *        ---
 *        type: entity
 *        ---
 ...
 *
 *   3. Inline wikilink lists without the outer brackets:
 *        related: [[a]], [[b]], [[c]]
 *      — semantically what the model wanted, but not valid YAML.
 *
 * Sanitizing on write means newly-generated files never need
 * read-time fallbacks, and re-ingesting an old file cleans it up
 * permanently.
 */
